...
class CoreEngineService:
    ...
    def get_stats(self):
        try:
            response = self.__stub.GetStatus(self.__manager._getRequestType("GetStatsRequest"))
            data = MessageToDict(
                response,
    ...
    def post_ingest_log(self, request: dict):
        try:
            ingest_request = ParseDict(request, self.__manager._getRequestType("IngestLogRequest"))
